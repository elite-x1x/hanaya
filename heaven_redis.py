...
if ADMIN_CHAT_ID == 0:
    logging.warning("⚠️ ADMIN_CHAT_ID tidak diatur")
...
# ============================================================
# Key lokal per bot
KEY_PENDING     = f"{BOT_NAME}:pending"
KEY_DAILY_COUNT = f"{BOT_NAME}:daily_count"
KEY_DAILY_DATE  = f"{BOT_NAME}:daily_date"
KEY_FLOOD_CTRL  = f"{BOT_NAME}:flood_ctrl"
KEY_DAILY_LIMIT = f"{BOT_NAME}:daily_limit"
KEY_SEND_DELAY  = f"{BOT_NAME}:send_delay"

# Key global (dibagikan antar semua bot)
KEY_GLOBAL_SENT = "heaven:global:sent"
...
DELAY_BETWEEN_GROUP_MAX = 50
WAIT_TIME               = 15
BATCH_PAUSE_EVERY       = 45
BATCH_PAUSE_MIN         = 120
BATCH_PAUSE_MAX         = 600
DAILY_LIMIT             = 2500
MAX_RETRIES             = 3
    ...
    def filter(self, record):
        if record.levelno >= logging.ERROR:
            ...
            if exc_info and isinstance(exc_info, tuple) and len(exc_info) >= 1:
                exc_type = exc_info
                exc_name = exc_type.__name__ if exc_type else "Unknown"  # ✅ __name__
...
async def is_duplicate(file_id: str, fp_hash: str) -> bool:
    # Cek di Redis global
    if await r_sismember(KEY_GLOBAL_SENT, file_id):
        return True
    if await r_sismember(KEY_GLOBAL_SENT, fp_hash):
        return True
...
async def mark_sent(file_id: str, fp_hash: str) -> None:
    # Simpan ke Redis global
    try:
        await r_sadd_with_ttl(KEY_GLOBAL_SENT, file_id, SENT_TTL)
        await r_sadd_with_ttl(KEY_GLOBAL_SENT, fp_hash, SENT_TTL)
    except Exception as e:
        logging.warning(f"⚠️ Redis global mark_sent error, fallback ke local: {e}")

    # Simpan ke local (fallback)
        ...
        try:
            ...
            # Chat tidak ditemukan / migrasi
                ...
                if migrated:
                    ...
                    await r_set("TARGET_CHAT_ID", str(new_id))
                    attempt += 1
                    continue
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        total_sent = await redis_client.scard(KEY_GLOBAL_SENT) if redis_client else len(local_sent)
    except Exception:
    ...
    text = (
        "📊 *Status Bot Heaven v5.0 — " + BOT_NAME + "*\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
