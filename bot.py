            ...
            if "flood" in err or "too many requests" in err or "retry" in err:
                try:
                    ...
                    total_wait = await flood_ctrl.record_flood(
                        ...
                        delay_min=DELAY_BETWEEN_GROUP_MIN,
                        delay_max=DELAY_BETWEEN_GROUP_MAX
                    )
                    
                    logging.warning(
                        f"⏳ [BOT: {BOT_NAME}] Flood wait {total_wait:.1f}s "
                        f"sebelum retry..."
                    )
        ...
        while True:
            try:
                ...
                if success:
                    try:
                        ...
                        try:
                            ...
                            logging.error(
                                f"❌ Error recording flood success: {e}"
                            )
                ...
                try:
                    await queue_manager.save_queue(pending_queue)
                    logging.info(
                        f"💾 [BOT: {BOT_NAME}] Queue saved: "
                        f"{pending_queue.qsize()} items"
                    )
                except Exception as e:
                    logging.error(
                ...
                try:
                    ...
                    logging.error(
                        f"❌ [BOT: {BOT_NAME}] Gagal save flood state: {e}"
                    )
    ...
    try:
        ...
        if role == "superadmin":
            text += (
                ...
                "/floodhistory - Lihat flood event history\n\n"
    ...
    try:
        ...
        if recovery.get("in_recovery"):
            recovery_str = (
                ...
                f"Est. {recovery['estimated_remaining']:.0f}s remaining\n"
            )
...
async def cmd_floodhistory(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if not flood_event_logger:
            await update.message.reply_text(
                "❌ Flood event logger tidak tersedia"
            )
            return
        ...
        if not recent_events:
            await update.message.reply_text(
                "✅ Tidak ada flood events dalam history"
            )
            return
        ...
        text = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"📊 Statistics:\n"
            f"├─ Total Events      : {flood_stats['total_events']}\n"
            f"├─ Avg Penalty       : {flood_stats['avg_penalty']:.0f}s\n"
            f"├─ Max Penalty       : {flood_stats['max_penalty']:.0f}s\n"
            f"├─ Events (1h)       : {flood_stats['events_last_hour']}\n"
            f"├─ Events (24h)      : {flood_stats['events_last_24h']}\n"
            f"└─ Last Event        : {flood_stats['last_event']}\n\n"
            f"Severity Distribution:\n"
            f"├─ 🔴 Critical       : {flood_stats['severity_distribution'] ['critical']}\n"
            f"├─ 🟠 High          : {flood_stats['severity_distribution'] ['high']}\n"
            f"└─ 🟡 Medium        : {flood_stats['severity_distribution'] ['medium']}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        for i, event in enumerate(recent_events, 1):
            ...
            text += (
                f"{i}. {severity_emoji} Event #{event['flood_count']}\n"
                f"   ├─ Time      : {time_str}\n"
                f"   ├─ Penalty   : {event['penalty']:.0f}s\n"
                f"   ├─ Total Wait: {event['total_wait']:.0f}s\n"
                f"   ├─ Failed    : {event['media_count']} media\n"
            )

            if event.get("error_msg"):
                text += f"   └─ Error    : {event['error_msg'] [:50]}\n"
            else:
                text += f"   └─ Error    : None\n"
        ...
        logging.error(f"❌ Error di cmd_floodhistory: {e}")
        await update.message.reply_text(f"❌ Error: {e}")
...
def health_check():
    ...
    try:
        ...
        try:
            ...
            global_info = global_sent_manager.get_info()
            reload_age = global_info.get("reload_age", "N/A")
        ...
        try:
            if pending_queue is not None:
                ...
                queue_usage = (
                    (queue_size / MAX_QUEUE_SIZE * 100)
                    if MAX_QUEUE_SIZE > 0 else 0
                )
                queue_unique = (
                    queue_deduplicator.get_queue_size() 
                    if queue_deduplicator else 0
                )
        ...
        response_data = {
            ...
            "uptime_formatted": (
                ...
                f"{int((uptime.total_seconds() % 3600) // 60)}m "
                f"{int(uptime.total_seconds() % 60)}s"
            ),
...
async def on_startup(app) -> None:
    ...
    global sending_lock, config_lock, ratelimit_lock, pending_queue, \
           flood_ctrl, duplicate_checker, queue_deduplicator, \
           _worker_task, _shutdown_event, console_mgr, \
           admin_command_queue, _admin_processor_task, \
           flood_event_logger, admin_flood_notifier
    ...
    try:
        ...
        try:
            flood_event_logger = FloodEventLogger(FLOOD_LOG_FILE, max_events=100)
            await flood_event_logger.load_from_file()
            logging.info(f"✅ [BOT: {BOT_NAME}] Flood event logger initialized")
        except Exception as e:
            logging.error(f"❌ [BOT: {BOT_NAME}] Gagal init flood logger: {e}")
        ...
        try:
            if ADMIN_CHAT_ID != 0:
                admin_flood_notifier = AdminFloodNotifier(
                    app.bot,
                    ADMIN_CHAT_ID
                )
                logging.info(
                    f"✅ [BOT: {BOT_NAME}] Admin flood notifier initialized "
                    f"(chat_id: {ADMIN_CHAT_ID})"
                )
            else:
                logging.warning(
                    f"⚠️ [BOT: {BOT_NAME}] ADMIN_CHAT_ID not set, "
                    f"flood notifications disabled"
                )
                admin_flood_notifier = None
        except Exception as e:
            logging.error(
                f"❌ [BOT: {BOT_NAME}] Gagal init flood notifier: {e}"
            )
...
def main():
    ...
    app.add_handler(CommandHandler("floodhistory", cmd_floodhistory))
