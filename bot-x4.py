...
for _i in range(1, 6):
    _val = os.getenv(f"MULTI_CHAT_ID_{_i}")
    if _val:
        try:
    ...
    while attempt < max_retries:
        try:
            ...
            if "flood" in err or "too many requests" in err or "retry" in err:
                # ✅ ADD SAFETY CHECK - PREVENT INFINITE LOOP
                if attempt >= max_retries:
                    logging.error(
                        f"❌ Max retries ({max_retries}) reached during flood, "
                ...
                try:
                    ...
                    # ✅ NOTIFY ADMIN JIKA FLOOD KRITIS
                    if flood_ctrl.flood_count > 2:
                        try:
                            msg = (
                                f"🚨 FLOOD DETECTED!\n\n"
                                f"├─ Count: {flood_ctrl.flood_count}\n"
                                f"├─ Wait: {total_wait:.0f}s\n"
                                f"├─ Penalty: {flood_ctrl.penalty:.0f}s\n"
                    ...
                    if attempt >= max_retries:
                        logging.error(
                            f"❌ Max retries reached after flood error, "
                            f"exiting retry loop"
                        )
                        return partial_success
